...
[tiles-performance](../tiles-performance/) で生成した最適化済み 3D Tiles と、[PLATEAU](https://www.mlit.go.jp/plateau/) が公式提供する 3D Tiles の表示パフォーマンスを、同一飛行ルートで比較するビューアアプリです。  
...
![最適化前の飛行](public/2.gif)

### 最適化後（改善済み 3D Tiles）

![最適化後の飛行](public/1.gif)
...
| | 最適化後（New） | 最適化前（Old） |
|---|---|---|
| **データソース** | [tiles-performance](../tiles-performance/) で生成 | PLATEAU 公式配信 |
...
| **maximumScreenSpaceError** | 5 | デフォルト（16） |
...
- [tiles-performance](../tiles-performance/) — 3D Tiles 生成・最適化スクリプト
